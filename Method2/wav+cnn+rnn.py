...
############################# Pre step1 #################################
    ...
    sample_x, sample_y = load_sample(main_path + paths[i], names[i], labels[i], sampling_rates[i], explanations[i],
                                     intervals[i], dimension=raw_dimension, step=3, train=False)
    if wav_dimension == 0:
...
############################# Step1 #################################
    ...
    cnn_train_x = np.empty((0, wav_dimension))
    ...
    for i in range(0,raw_train_x.shape[0]):
        ...
        cnn_train_x = np.append(cnn_train_x, reshaped_features, axis=0)
    cnn_train_y = raw_train_y
    cnn_train_x = np.reshape(cnn_train_x,[cnn_train_x.shape[0], cnn_train_x.shape[1],1])

    print('Build model...')
    cnn_model = Sequential()
    cnn_model.add(Conv1D(filters=nb_filter1, kernel_size=filter_length1, activation='relu', input_shape=(wav_dimension, 1)))
...
############################ Step 2 ################################
    ...
    for i in train_samples_id:
        ...
        predicted = cnn_model.predict(sample_features)
...
# ----------------  Test model  on test samples ---------------
    ...
    for i in test_samples_id:
        ...
        step1_predicted = cnn_model.predict(sample_features)
...
text_file = open("Result_wav+CNN+RNN.txt", "a")
text_file.write("\t\t\tResult wavelet + CNN + RNN\n")
text_file.write("\n################### Save results & parameters in file ####################\n")
